    ...
    if arg.is_some() {
        ...
        if result.is_some() {
            if !prog_option.output_file.is_empty() {
                ...
                let res: HashMap<String, Rgb<u8>> =
                    get_closest_color_ver2(&hex_to_pixel(&ru), &prog_option);
                for (name, color) in res.iter() {
                    buffer.push_str(&format!("{} = \"{:06x}\"\n", name, pixel_to_hex(color)));
                }
                let _ = fs::write(prog_option.output_file.clone(), buffer.as_bytes()).map_err(|_| {
                    eprintln!("ERROR: Failed to write to {}, please check the permission or storage size.", prog_option.output_file);
                    return;
                });
                return;
            }
            let res: HashMap<String, Rgb<u8>> =
                get_closest_color_ver2(&hex_to_pixel(&result.unwrap()), &prog_option);
            for (name, color) in res.iter() {
